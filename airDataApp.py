...
# MONTHLY REPORT SECTION
    ...
    # Main Pollutant by Month
    # Main Pollutant by Month (Pie Chart)
    fig_pollutant_month = px.pie(
        filtered_monthly,
        names="mainPollutantMonthly",  # The pollutant categories (e.g., Ozone, PM2.5, etc.)
        title=f"Main Pollutant by Month for {selected_city}",
        color="mainPollutantMonthly",  # Color each pollutant category distinctly
        color_discrete_map={  # Color mapping for pollutants
        'Ozone': 'lightblue',
        'PM2.5': 'darkred',
        'CO': 'darkbrown',
        'PM10': 'darkorange',
        ...
        labels={"mainPollutantMonthly": "Pollutant"}  # Customize the label for clarity
    )
...
# Display the pie chart
    ...
    fig_aqi_category_month = px.pie(
        filtered_monthly,
        names="AQI category",  # The AQI categories (e.g., Good, Moderate, etc.)
        title=f"Main AQI Category by Month for {selected_city}",
        color="AQI category",  # Color each AQI category distinctly
        color_discrete_map={  # Color mapping for AQI categories
        'Good': 'green',
        'Moderate': 'yellow',
        ...
        labels={"AQI category": "AQI Category"}  # Customize the label for clarity
    )
...
# ANNUAL REPORT SECTION
    ...
    # Main AQI Category by City (Annual)
   # Main AQI Category by City (Annual) - Pie Chart
    fig_aqi_category_ann = px.pie(
        annual_data,
        names="mainAQICategoryAnnual",  # Category of AQI (e.g., Good, Moderate, etc.)
        title="Main AQI Categories Annually",
        color="mainAQICategoryAnnual",  # To color each category distinctly
        color_discrete_map={  # Color mapping for categories
            'Good': 'green',
            'Moderate': 'yellow',
            'Unhealthy for Sensitive Groups': 'orange',
            'Unhealthy': 'red',
        ...
        labels={"mainAQICategoryAnnual": "AQI Category"}  # Label the categories
    )
